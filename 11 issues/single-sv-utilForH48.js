...
javascript:(function(){
	...
	/* 具体処理をprototypeで定義 */
	utilForH48.prototype = {
		/* メソッドを定義、1個だけなのでexecと命名 */
		exec: function() {
			var key = this.util.get_survey_key(this.sv);
			this.util.set_survey(key);
			this.util.set_survey_copy("no");
			this.util.set_comment("リストの視覚表現を取っている箇所に適当な要素を用いていない");
			if(this.util.get_srccode()==="") this.util.set_srccode(this.util.get_description());
			var old_src = this.util.get_srccode();
			old_src=old_src.replace(/^ +/m,"");
		    old_src=old_src.replace(/\t+/m,"");
		    if(!this.pt.test(old_src)) return;
		    var html_str = "<ul>";
		    var arr = old_src.split(this.pt);
		    for(var i=0; i<arr.length; i++) {
		    	if(!new RegExp(/(\n|\r|\r\n)/).test(arr[i])) {
		    		html_str += "<li>" + arr[i] + "</li>" + "\r\n";
				}
		    }
		    html_str += "</ul>";
			this.util.set_srccode(html_str);
		}
	};
	/* オブジェクトをインスタンス化して実行 */
	var app = new utilForH48();
	app.exec();
	/* ---- サンプルコードここまで ---- */
