...
javascript:(function(){
	...
	var sv_key = util.get_survey_key(sv);
	util.set_survey(sv_key);
	if(sv==="非適用") util.set_comment("判定すべき対象コンテンツが使用されていない");
	if(sv==="適合") {
		if(util.get_srccode()!=="") util.set_srccode("");
	}
	if(autosave==="yes") util.save();
