...
javascript:(function() {
	...
	function trgr(guidelines, surveyarr) {
		...
		my_html += '<label><input type="checkbox" id="uncomp_guideline_flag" value="1">達成基準別</label>';
		...
		if(document.getElementById("mycss") == null) {
			...
			mycss_str += 'z-index:2999;width:700px;height:250px;';
			mycss_str += '}';
			mycss_str += '#CheckTool_diag button { font-size: 99%; }';
			mycss_str += '#reportArea { boder:1px solid #ccc; width: 700px; height: 200px; overflow: scroll;}';
			mycss_str += '#reportTable { width: 700px; border-collapse:collapse; table-layout: fixed;';
			mycss_str += 'word-break: break-all;}';
			mycss_str += '#reportTable tbody tr:first-child td:nth-child(1),';
			mycss_str += '#reportTable tbody tr:first-child td:nth-child(2),';
			mycss_str += '#reportTable tbody tr:first-child td:nth-child(3)';
			mycss_str += '{ width: 50px; }';
			mycss_str += '#reportTable tbody tr:first-child td:nth-child(4)';
			mycss_str += '{ width: 80px; }';
		...
		if(document.getElementById("myjs") == null) {
			...
			myjs_str += 'var target = document.getElementById("uncomp_guideline_flag");';
			myjs_str += 'if(target.checked === true) {';
			myjs_str += 'do_marking_uncomp_by_guideline();';
			myjs_str += '} else {';
			myjs_str += 'var tbl = document.getElementsByTagName("table").item(2);';
			myjs_str += 'var tr = tbl.rows;';
			myjs_str += 'for(var i=0; i<tr.length; i++) {';
			...
			myjs_str += '}';
			myjs_str += '}';
			myjs_str += 'function do_marking_uncomp_by_guideline() {';
			myjs_str += 'var select1 = document.getElementById("pup_guideline_id");';
			myjs_str += 'var target = select1.options[select1.selectedIndex].text;';
			myjs_str += 'var idx = tblmap[target];';
			myjs_str += 'var tbl = document.getElementsByTagName("table").item(2);';
			myjs_str += 'var tr = tbl.rows;';
			myjs_str += 'for(var i=0; i<tr.length; i++) {';
			myjs_str += 'if(i < 2) continue;';
			myjs_str += 'var row = tr.item(i);';
			myjs_str += 'var first_cell = row.cells.item(0);';
			myjs_str += 'var cell = row.cells.item(idx);';
			myjs_str += 'var cell_data = cell.getElementsByTagName("a").item(0).innerHTML;';
			myjs_str += 'if(cell_data == "－") {';
			myjs_str += 'first_cell.setAttribute("style", "background:yellow");';
			myjs_str += '}';
